    ...
    try {
      switch (true) {
        ...
        case command === '/trades': {
          ...
          const trades = lastDashboardData.recentTrades.slice(0, 5);
          if (trades.length === 0) {
            await sendTelegramMessage('📋 _No trades yet._', { chatId });
            break;
          }

          let msg = '📋 *RECENT TRADES (Last 5)*\n' + '─'.repeat(30) + '\n';
...
async function runTick() {
  ...
  try {
    ...
    console.log(`[${new Date().toISOString()}] tick ✓ | BTC=${price} | Balance=${balance} | ${status}`);
  } catch (error) {
...
console.log('  🤖 GHOST RUN TRADING ENGINE');
console.log(`  📡 Server: ${appUrl}`);
console.log(`  ⏱️  Tick Interval: ${tickIntervalMs}ms`);
console.log(`  📲 Telegram: ${isTelegramEnabled() ? 'ENABLED' : 'WAITING FOR CHAT ID'}`);
console.log('═'.repeat(50));
