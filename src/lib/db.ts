...
export function initDB() {
  ...
  if (!stateRow) {
    db.prepare(`
      ...
      VALUES (1, 0.05, 0.003, 0.004, 0.0015, 0.0025, 0, 0, 1.0, 10000.0, 0.30)
    `).run();
    /*
     * HYPER-AGGRESSIVE CONTINUOUS TRADING PARAMETERS:
     * confidence_threshold: 0.05 = practically trade on any directional bias
     ...
     * max_daily_loss_percentage: 1.0 = 100% (never stop trading due to losses)
     * daily_profit_target: 10000.0 = $10,000 daily profit target (never stop due to wins)
     */
  }
  ...
  if (stateRow) {
    ...
    if (existing.confidence_threshold >= 0.15 || existing.profit_target_percentage >= 0.005) {
      db.prepare(`
